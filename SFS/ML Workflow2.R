...
probeSM <- read.csv(paste0(dataRoot, '2/ProbeDumps/SM_All.csv'), stringsAsFactors = F)

#sDate <- as.Date('2015-11-19')
sDate <- as.Date('2019-06-08')
eDate <- as.Date('2019-07-19')
...
head(covsDrill)
write.csv(covsDrill, paste0(dataRoot, '2/Drills/covsDrill.csv'), row.names = F)
...
for (i in 1:length(smipsfls)) {
  ...
  c <- rbind(simpsDrillDF, outsmpDF)
}

colnames(simpsDrillDF) <- c('sid', 'dt', 'SMIPSVal')
write.csv(simpsDrillDF, paste0(dataRoot, '2/Drills/smipsDrill.csv'), row.names = F)

covsnpDF <- merge(simpsDrillDF, covsDrill, 'sid')
head(covsnpDF)
sdf <- covsnpDF[order(covsnpDF$sid, covsnpDF$dt),] 
head(sdf)
write.csv(sdf, paste0(dataRoot, '2/Drills/smipsAndCovsMerge.csv'), row.names = F)
...
csDF <- read.csv(paste0(dataRoot, '2/Drills/smipsAndCovsMerge.csv'), stringsAsFactors = F)
...
write.csv(otdf2, paste0(dataRoot, '2/Drills/rawTraining.csv'), row.names = F)
...
modDf <- read.csv(paste0(dataRoot, '2/Drills/rawTraining.csv'), stringsAsFactors = F)
